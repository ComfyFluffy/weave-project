...
export function createWorldStateRoutes(
  ...
  emitWorldStateUpdate?: (worldStateId: string, worldState: any) => void
) {
  const router = Router()
  ...
  router.get(
    ...
    async (
      req: Request<{ id: string; characterId: string }>,
      res: Response
    ) => {
      try {
        const worldState = await dbService.getWorldStateById(req.params.id)
  ...
  router.delete(
    ...
    async (
      ...
      try {
        ...
        // Remove character
        const updatedCharacters = worldState.characters.filter(
          (c) => c.id !== req.params.characterId
        )
        const updatedWorldState = {
          ...worldState,
  ...
  router.put(
    ...
    async (
      req: Request<{ id: string; locationName: string }>,
      res: Response
    ) => {
      try {
        const worldState = await dbService.getWorldStateById(req.params.id)
  ...
  router.put(
    '/:id/plots/:plotTitle',
    async (req: Request<{ id: string; plotTitle: string }>, res: Response) => {
      try {
        const worldState = await dbService.getWorldStateById(req.params.id)
        if (!worldState) {
          return res.status(404).json({ error: 'World state not found' })
